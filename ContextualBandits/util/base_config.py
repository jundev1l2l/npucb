...
class BaseConfig:
    """
    Custom Config
    - need replacement by well-known config managment tool
    """
    ...
    def __call__(self):
        ...
        for key, value in config_dict.items():
            if isinstance(value, list) and (self.subconfig_class_hash[key] != list):
                config_dict[key] = [v() if "Config" in type(v).__name__ else self.subconfig_class_hash[key](v) for v in value]
        ...
        if isinstance(subconfig_class, dict):
            ...
            if isinstance(value, list) and (subconfig_class != list):
                ...
                if key in self.subconfig_class_hash.keys():
                    try:
                        if subconfig_class == list and type(value) == int:  # todo: check if this added feature is clean
                            value = [value,]
