...
from util.misc import find_free_port
...
def main():
    ...
    if not hasattr(config.engine, "init_method"):
        config.engine.init_method = f"tcp://127.0.0.1:{find_free_port()}"
    if hasattr(config, "dataloader") and not hasattr(config.dataloader, "num_workers"):
        config.dataloader.num_workers = 4 * len(config.engine.gpu)
